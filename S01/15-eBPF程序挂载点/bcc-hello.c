// clang bcc-hello.c -o bcc-hello -lbcc_bpf
...
int main(int argc, char **argv)
{
    ...
    printf("insn_cnt: %d\n", insn_cnt);
    int prog_fd = bcc_prog_load(BPF_PROG_TYPE_KPROBE, "hello",
                                   (const struct bpf_insn *)&prog,
                                   insn_cnt * sizeof(struct bpf_insn),
                                   "GPL",
                                   LINUX_VERSION_CODE,
                                   1, bpf_log_buf, BPF_LOG_BUF_SIZE);
    if (prog_fd < 0)
    {
    ...
    int kprobe_fd = bpf_attach_kprobe(prog_fd, BPF_PROBE_ENTRY, "kprobe__hello", "do_unlinkat", 0, 0);
    if (kprobe_fd < 0)
    {
